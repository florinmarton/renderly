...
export type AdStudioLayerFeedPropertyBind = { hash: string; column: string; index?: string } | null;
export type AdStudioLayerFeed = Record<string, AdStudioLayerFeedPropertyBind | AdStudioLayerFeedPropertyBind[]>;

export type JsonPropertiesFeed = EditorLayerFeed | AdStudioLayerFeed;

export function isEditorLayerFeed(variable: JsonPropertiesFeed): variable is EditorLayerFeed {
  return typeof variable === 'string' && variable !== null;
}

export function isAdStudioLayerFeed(variable: JsonPropertiesFeed): variable is AdStudioLayerFeed {
  return (
      variable !== null &&
      ((typeof variable === 'object' && 'hash' in variable && 'column' in variable) || Array.isArray(variable))
  );
}
...
export function isEditorDesignFeed(variable: JsonDesignFeed): variable is EditorDesignFeed {
  return typeof variable === 'object' && variable !== null && 'hash' in variable;
}

export function isAdStudioDesignFeed(variable: JsonDesignFeed): variable is AdStudioDesignFeed {
  return typeof variable === 'object' && variable !== null && 'hashes' in variable;
}
...
export type JsonAlign = 'bottom' | 'center' | 'left' | 'middle' | 'right' | 'top';

export type JsonScaleMode = 'stretch' | 'aspect' | 'mask' | 'tile' | 'crop' | 'userCrop';
...
export type JsonBackgroundSolidText = JsonBackgroundSolid & { feed?: AdStudioLayerFeedPropertyBind };
...
export function isJsonBackgroundNone(bg: JsonBackground | undefined): bg is JsonBackgroundNone {
  return bg !== undefined && bg && bg.type === 'none';
}

export function isJsonBackgroundSolid(bg: JsonBackground | undefined): bg is JsonBackgroundSolid {
  return bg !== undefined && bg && bg.type === 'solid';
}

export function isJsonBackgroundLinearGradient(bg: JsonBackground | undefined): bg is JsonBackgroundLinearGradient {
  return bg !== undefined && bg && bg.type === 'lgrad';
}

export function isJsonBackgroundRadialGradient(bg: JsonBackground | undefined): bg is JsonBackgroundRadialGradient {
  return bg !== undefined && bg && bg.type === 'rgrad';
}

export function isJsonBackgroundImage(bg: JsonBackground | undefined): bg is JsonBackgroundImage {
...
export function isSlide(element: JsonSlideOrElement | JsonDesign): element is JsonSlide {
  return (element as JsonSlide).type === 'slide';
}

export function isElement(element: JsonSlideOrElement | JsonDesign): element is JsonElement {
  return (element as JsonElement).type === 'layer';
}

export function isDesign(element: JsonSlideOrElement | JsonDesign): element is JsonDesign {
...
export function isWeather(element: JsonElement): element is JsonRain | JsonSnow | JsonFog {
  return ['fog', 'rain', 'snow'].includes(element.layerType);
}

export function isMedia(element: JsonElement): element is JsonElementMedia {
  return isImage(element) || isSvg(element) || isVideo(element) || isAudio(element);
}

export function hasAnimation({ properties: { buildIn, buildMid, buildOut } }: JsonElement) {
  if (!buildOut || !buildMid || !buildIn) {
      return false;
  }

  return buildIn.type !== 'none' || buildMid.type !== 'none' || buildOut.type !== 'none';
}

export function isAnimated(element: JsonElement): element is JsonAnimatedElement {
  return (
...
export type JsonEaseType = 'linear' | 'ease' | 'easeIn' | 'easeOut' | 'easeInOut';
...
export type JsonAnimatedElement = JsonVideo | JsonAudio | JsonYoutube | JsonEmbed;
...
export const JsonFlipTypeArr = ['both', 'horizontal', 'none', 'vertical'] as const;
...
export type JsonTextVerticalAlignmentType = (typeof JsonTextVerticalAlignment)[keyof typeof JsonTextVerticalAlignment];
...
export type JsonTextContentBoundaryType = (typeof JsonTextContentBoundary)[keyof typeof JsonTextContentBoundary];

export const JsonTextContentHeight = {
  auto: 'auto',
  fixed: 'fixed',
} as const;
export type JsonTextContentHeightType = (typeof JsonTextContentHeight)[keyof typeof JsonTextContentHeight];
...
export type JsonAction = JsonActionGotoURL | JsonActionGotoSlide | JsonActionNone | JsonActionVideoEnd;

export function isJsonActionGotoSlide(action: JsonAction): action is JsonActionGotoSlide {
  return action.type === 'gotoSlide';
}

export function isJsonActionGotoURL(action: JsonAction): action is JsonActionGotoURL {
...
export function isSlateConfigChildren(
  config: JsonTextSlateConfigChildren | JsonTextSlateConfigNode | JsonTextOldConfig,
...
export function isSlateConfig(config: JsonTextSlateConfig | JsonTextOldConfig[]): config is JsonTextSlateConfig {
  return 'type' in config && 'nodes' in config;
}

export function isTextOldConfig(config: JsonTextSlateConfig | JsonTextOldConfig): config is JsonTextOldConfig {
  return 'style' in config && 'children' in config;
}

export function isOldTextV1(node: JsonText | JsonOldTextV2 | JsonOldTextV1): node is JsonOldTextV1 {
  return !('type' in node);
}

export function isOldSetText(node: JsonTextSet | JsonTextOldConfigStyle): node is JsonTextOldConfigStyle {
...
export function isJsonSetImage(element: JsonSetElement): element is JsonImageSet {
  return element.layerType === 'image';
}

export function isJsonSetVideo(element: JsonSetElement): element is JsonVideoSet {
  return element.layerType === 'video';
}
...
export function isJsonSetButton(element: JsonSetElement): element is JsonButtonSet {
  return element.layerType === 'button';
}

export function isJsonSetShape(element: JsonSetElement): element is JsonShapeSet {
  return element.layerType === 'shape';
}
...
export function isJsonDesignSet(element: JsonDesignSet | JsonSetElement): element is JsonDesignSet {
  return 'elements' in element && !('layerType' in element) && !('elementType' in element);
}
...
// Custom types
export type DesignJsonWithBanner = {
  hash: string;
  banner: JsonDesign;
};
